...
> = ({
		...
		if (menuPosition.top !== null && menuPosition.left !== null) {
			...
			if (e.key === "ArrowDown" || e.key === "ArrowUp") {
				...
				setActiveMenuItemId((prevId) => {
					...
					if (newId >= internalMenuItems.length)
						return internalMenuItems.length - 1; // Prevent going above the last item
					return newId;
				});
			} else if (e.key === "Enter") {
				e.preventDefault();
				if (
					...
					internalMenuItems[activeMenuItemId]
				) {
	...
	const internalOnChange = (evt: FormEvent<HTMLDivElement>) => {
		...
		const strippedContentUpToCursor = sanitizeHtml(
			innerHTML.slice(0, position),
			strictHTMLSanitization
		);

		const lastSearchTerm = strippedContentUpToCursor
			.split(/<[^>]*>|\s+/gm)
			.pop()
			?.startsWith(searchTrigger)
			? strippedContentUpToCursor
					.split(/<[^>]*>|\s+/gm)
					.pop()
					?.slice(searchTrigger.length)
					.trim() ?? ""
			: undefined;
		...
		debounce(() => {
			// If the last search term is empty, remove the menu
			if (strippedContent) {
	...
	const internalOnSelect = (item: MenuClass.Item) => {
		...
		if (contentEditableRef.current) {
			...
			const contentBeforeCursor = currentContent.slice(
				0,
				internalCursorPosition
			);
			// Replace the last search term with the selected item
			const lastSearchTermIndex =
				contentBeforeCursor.lastIndexOf(searchTrigger);
			const contentBeforeSearchTrigger = contentBeforeCursor.slice(
				0,
				lastSearchTermIndex
			);
			// Construct the new content
			const contentAfterCursor = currentContent.slice(internalCursorPosition);
			const newContent = `${contentBeforeSearchTrigger}${TagWrapper(
				(preserveSearchTrigger ? searchTrigger : "") + item.label,
				showSelectionAsHTMLTag
			)}${contentAfterCursor}`;
			contentEditableRef.current.innerHTML = sanitizeHtml(
				newContent,
				strictHTMLSanitization
			);
			...
			if (showSelectionAsHTMLTag === false) {
				...
				Array.from(contentEditableRef.current.childNodes).forEach(
					(node, id) => {
						if (
							node.textContent?.indexOf(item.label) !== -1 &&
							contentEditableRef.current
						) {
							targetChild = Array.from(contentEditableRef.current.childNodes)?.[
								id + 1
							]; // +1 because target is the text node following the custom node (default to anchor) containing '&nbsp;'
